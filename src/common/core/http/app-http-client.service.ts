...
export class AppHttpClient {
    ...
    apiUrl='https://server.visionvivante.com:8090/bemusic/'
    constructor(protected httpClient: HttpClient, protected errorHandler: HttpErrorHandler) {}
    public checkUrl<T>(uri: string, params = {}, options: object = {}): Observable<T> {
        let url=uri.split('secure/');
        const httpParams = this.transformQueryParams(params);
        return this.httpClient.get<T>(url[0]+this.prefixUri(url[1]), {params: httpParams, ...options})
            .pipe(catchError(err => this.errorHandler.handle(err, uri, options)));
    }
    public get<T>(uri: string, params = {}, options: object = {}): Observable<T> {
        const httpParams = this.transformQueryParams(params);
        return this.httpClient.get<T>(this.apiUrl+this.prefixUri(uri), {params: httpParams, ...options})
            .pipe(catchError(err => this.errorHandler.handle(err, uri, options)));
    }

    public post<T>(uri: string, payload: object = null): Observable<T> {
        return this.httpClient.post<T>(this.apiUrl+this.prefixUri(uri), payload)
    ...
    protected prefixUri(uri: string) {
         console.log('https://server.visionvivante.com:8090/bemusic/'+uri)

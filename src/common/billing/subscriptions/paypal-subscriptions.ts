...
export class PaypalSubscriptions {
     ...
     */
    public subscribe(params: {plan_id: number, start_date?: string},user_id:any): Promise<User> {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.createPaypalAgreement(params,user_id).subscribe(response => {
     ...
     */
    public changePlan(subscription: Subscription, plan: Plan, user_id:any): Promise<User> {
        return new Promise(resolve => {
            this.subscriptions.cancel(subscription.id, {delete: true}).subscribe(() => {
                this.subscribe({plan_id: plan.id, start_date: subscription.renews_at}, user_id).then(user => resolve(user));
            });
        });
    }
    ...
    private createPaypalAgreement(params: {plan_id: number, start_date?: string},user_id): Observable<{urls: {execute: string, approve: string}}> {
        return this.http.post('billing/subscriptions/paypal/agreement/create/'+user_id, {plan_id: params.plan_id, start_date: params.start_date});
    }

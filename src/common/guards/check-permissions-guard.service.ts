...
export class CheckPermissionsGuard implements CanActivate, CanActivateChild {
    ...
    private checkPermissions(route: ActivatedRouteSnapshot, state: RouterStateSnapshot) {
        ...
        // }
        if(this.currentUser.hasRole('admin') || this.currentUser.hasRole('moderator')){
            return true;
        }
        ...
        if ( ! this.currentUser.isLoggedIn()) {
            ...
            const uri = this.shouldRedirectToUpgrade(route.data.permissions) ? '/web/billing/upgrade' : this.auth.getRedirectUri();

...
export class MediaImageComponent implements OnChanges {
    ...
    private getSrc(media: ChannelContentItem|string) {
        ...
        switch (media.model_type) {
            case ARTIST_MODEL:
                return environment.image_base_url+(media as Artist).image_small || environment.image_base_url+DefaultImagePaths.artistSmall;
            case MIXED_ARTIST_MODEL:
                return environment.image_base_url+(media as MixedArtist).image || environment.image_base_url+DefaultImagePaths.artistSmall;
            case ALBUM_MODEL:
                return this.albumImage(media as Album);
            case TRACK_MODEL:
                return this.trackImage(media as Track);
            case USER_MODEL:
                return environment.image_base_url+(media as User).avatar;
            case PLAYLIST_MODEL:
                return environment.image_base_url+(media as Playlist).image || environment.image_base_url+DefaultImagePaths.album;
            case CHANNEL_MODEL:
                return environment.image_base_url+DefaultImagePaths.album;
            case GENRE_MODEL:
                return environment.image_base_url+(media as Genre).image || environment.image_base_url+DefaultImagePaths.artistSmall;
        }
    }

    public albumImage(album: Album): string {
        if (album && album.image) return environment.image_base_url+album.image;
        return environment.image_base_url+DefaultImagePaths.album;
    }

    public trackImage(track: Track) {
        if (track.image) {
            return environment.image_base_url+track.image;
        } else if (track.album && track.album.image) {
            return environment.image_base_url+track.album.image;
        } else {
            return environment.image_base_url+DefaultImagePaths.album;
        }
    }
    ...
    public defaultImage() {
        return environment.image_base_url+DefaultImagePaths[this.default];
    }

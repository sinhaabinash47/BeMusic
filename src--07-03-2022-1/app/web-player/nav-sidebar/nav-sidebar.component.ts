...
export class NavSidebarComponent implements OnInit {
    ...
    getSubGenres(){
        this.auth.getSubGenres().subscribe((res:any)=>{
            if(res.status == 'success'){
                this.allSubGenres = res.pagination.data;
            }
        })        
    }
    // getGenre(){
    ...
    getsearchcus(event){
        ...
        paramsdata['query'] = event.target.value;
    ...
    getGenresearch(event){
        ...
        paramsdata['genre'] = event.target.value;
        this.router.navigateByUrl('/', {skipLocationChange: true}).then(() => {
            this.router.navigate(['web/dashboard'], { queryParams: paramsdata});
        });
    }
    getsubGenresearch(event){
        ...
        paramsdata['subgenre'] = event.target.value;
        this.router.navigateByUrl('/', {skipLocationChange: true}).then(() => {
            this.router.navigate(['web/dashboard'], { queryParams: paramsdata});
        });
    }
    getGenre(event){
        ...
        paramsdata['catid'] = event.target.value;
        paramsdata['genre'] = 0;
        paramsdata['subgenre'] = 0;
        this.router.navigateByUrl('/', {skipLocationChange: true}).then(() => {
            this.router.navigate(['web/dashboard'], { queryParams: paramsdata});
        });
    }
